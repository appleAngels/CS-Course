...
![图中是我们实际系统里性能监测工具 NewRelic 中的响应时间，代表了每个外部的 Web 请求的执行时间](./img/03-02.png)

<center>图中是我们实际系统里性能监测工具 NewRelic 中的响应时间</center>

<center>代表了每个外部的 Web 请求的执行时间</center>

第二个是**吞吐率**（Throughput）或者带宽（Bandwidth），想要提升这个指标，你可以理解为让计算机“搬得更多”。

![服务器使用的网络带宽，通常就是一个吞吐率性能指标](./img/03-03.png)

<center>服务器使用的网络带宽，通常就是一个吞吐率性能指标</center>
...
<center>一份 SPEC 报告通常包含了大量不同测试的评分</center>
...
<center>程序实际占用的 CPU 时间一般比 Elapsed Time 要少不少</center>
...
<center>程序的 CPU 执行时间 =CPU 时钟周期数×时钟周期时间</center>
